...
try {
  ...
  await page.goto(
    `${base}/iframe.html?id=astro-tag-profilebadge--default&viewMode=story`,
  );
  await page.getByText("Fish lover").waitFor();
  await page.locator("svg").first().waitFor();

  await page.goto(
    `${base}/iframe.html?id=astro-section-hero--default&viewMode=story`,
  );
  ...
  console.log(
    "Static Astro Icon stories rendered in Chromium without page errors",
  );
} finally {
  await browser?.close();
